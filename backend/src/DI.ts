...
import { Certificate, Credentials, User } from "./types/types";
...
export class DecentralizeIdentity {
  ...
  constructor(signer: JsonRpcSigner | Signer, credentials: Credentials) {
  ...
  getUserData = async (did: string): Promise<User> => {
    const user_data = await this.contract.get_user_data(did);
    return user_data;
  };
  ...
  getOwnedCertificatesData = async (did: string) => {
    try {
      const user_owned_certificates_uris = (
        await this.contract.get_user_data(did)
      ).owned_certificates;
